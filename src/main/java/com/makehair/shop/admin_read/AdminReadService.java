...
public class AdminReadService {
    ...
    public Map<String, Object> getDataList(ReadVo readVo) {
        PageVo pageVo = pageService.getPaging(readVo.getReadType(), readVo.getSearchType(), readVo.getSearchValue(), readVo.getPageNum(), readVo.getAdminNo());
        ...
        if(readVo.getReadType().equals("salesDay") || readVo.getReadType().equals("salesMonth")) {
            Map<String, Object> tmpMap = DateFormatUtil.getStartEndTime(readVo.getReadType(), readVo.getSearchValue());
            map.put("startTime", tmpMap.get("startTime"));
            map.put("endTime", tmpMap.get("endTime"));
