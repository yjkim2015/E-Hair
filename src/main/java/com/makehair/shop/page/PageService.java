...
public class PageService {
    ...
    public PageVo getPaging(String readType, String searchType, String searchValue, int pageNum, int adminNo) {
        ...
        System.out.println("this is readtype in pageService : " + readType);
        ...
        } else if(readType.equals("review")) {
            map.put("searchValue", adminNo);
            count = adminReadDao.getReviewCount(map);
        } else {
            // 메모
            map.put("searchValue", Integer.parseInt(searchValue));
            count = adminDao.getMemoCount(map);
        }
