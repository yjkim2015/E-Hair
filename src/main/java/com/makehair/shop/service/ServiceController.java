...
public class ServiceController extends CommonController {
    ...
    @RequestMapping(value = "/service/delete/{serviceNo}", method = RequestMethod.DELETE)
    @ResponseBody
    public ResponseEntity<Boolean> deleteService(@PathVariable("serviceNo") Integer serviceNo) {

        return ResponseEntity.ok().body(serviceDao.delete(serviceNo) == 1);
    }
}
